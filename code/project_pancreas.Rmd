...
ggplot(plot_data_pancreas75, aes(x=age, y = estimate/12, color=race))+
  ...
  xlab("Age")+
  theme_light()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size = 32),
        axis.text.y = element_text(size = 32),
        axis.title = element_text( size = 32),
        axis.title.y = element_text(hjust = 0.5),
        strip.text = element_text(size = 32),
...
Stategy plot (This is old and is replaced with a new version below)
...
Updated strategy plot

```{r}
strategy_pancreas3 <- pancreas_data_survival[,c("Age_Cat", "Sex", "Race_Origin", "Year_Diagnosis", "estimate_screened_all_75")] %>% filter(Year_Diagnosis == 2010)

#take the expected lifetime, multiply it by the number of people with pancreas cancer divided by the population of the united states for a given age and race category
strategy_pancreas3$age55 <- ifelse(strategy_pancreas3$Age_Cat < 55, "Under 55", "55+")
strategy_pancreas3$population <-  ifelse(strategy_pancreas3$Race_Origin == "Hispanic" & strategy_pancreas3$age55 == "55+", 5724831,
                                  ifelse(strategy_pancreas3$Race_Origin == "Hispanic" & strategy_pancreas3$age55 == "Under 55", 16886751,
                                  ifelse(strategy_pancreas3$Race_Origin == "Caucasian" & strategy_pancreas3$age55 == "55+", 54891829,
                                  ifelse(strategy_pancreas3$Race_Origin == "Caucasian" & strategy_pancreas3$age55 == "Under 55", 68000019,
                                  ifelse(strategy_pancreas3$Race_Origin == "African American" & strategy_pancreas3$age55 == "55+", 6843792,
                                  ifelse(strategy_pancreas3$Race_Origin == "African American" & strategy_pancreas3$age55 == "Under 55", 13145040,
                                  ifelse(strategy_pancreas3$Race_Origin == "Asian or Pacific Islander" & strategy_pancreas3$age55 == "55+",
                                  2861883, 5929757)))))))
for(i in 1:nrow(strategy_pancreas3))
{
  strategy_pancreas3$numb[i] <- nrow(strategy_pancreas3 %>% filter(Race_Origin == strategy_pancreas3$Race_Origin[i]) %>% filter(age55 == strategy_pancreas3$age55[i]))
}
strategy_pancreas3$estimate_dist<-strategy_pancreas3$estimate_screened_all_75*strategy_pancreas3$numb/strategy_pancreas3$population
strategy_pancreas3$group <- paste(strategy_pancreas3$Race_Origin, strategy_pancreas3$age55)
```

```{r, fig.width = 10}
ggplot(strategy_pancreas3, aes(group, estimate_dist/12*365, fill = Race_Origin)) + geom_violin(size = 1, scale = "width", trim = FALSE)+ geom_boxplot(width = 0.1, fill = "white")+facet_wrap(~age55, scale = "free")+
  labs(y = "Survival Time Per Screen (Days)",
       x = "Early Screening Strategy",
       fill='Ancestry:',
       linetype = "Age Category") + 
  # scale_linetype_manual(values = c("dotted", "solid"))+
  # scale_size_manual(values = c(2, 0.5), guide = "none")+
  scale_fill_manual(values=c("#009E73", "#56B4E9", "#F0E442", "#E69F00", "#0072B2", "#D55E00", "#CC79A7"))+
    theme_light()+
...
df_pancreas2 <- matrix(0, length(seq(0, 1, by = 0.01))*length(seq(0, 1, by = 0.01)), 3)
t <- 1
...
ggplot(as.data.frame(df_pancreas2), aes(x = V1, y = V2, fill = V3/12))+
  ...
  scale_y_continuous(expand = c(0,0), breaks = seq(0.25, 1, by = 0.25))+
  theme(axis.text.x = element_text(size = 32),
        strip.text = element_text(size = 32),
        axis.title = element_text( size = 32),
        axis.text.y = element_text(size = 32),
...
ggplot(as.data.frame(df_pancreas3))+
  ...
  low = "red",
  high = "blue", breaks = seq(0, 0.75, by = 0.25)
)+
  theme_light()+
...
df_pancreas4 <- matrix(0, length(seq(0, 1, by = 0.01))*length(seq(0, 0.001, by = 0.0001)), 4)
t <- 1
for(x in seq(0, 0.001, by = 0.0001))
{
  for(a in seq(0, 1, by = 0.01))
  {
    data_filtered <- df_pancreas %>% filter(Race_Origin == "Caucasian") %>% filter(Age_Cat == "55-59 years") %>% filter(Sex == "Male")
    ...
    for(e in 1:nrow(data_filtered))
    {
      ...
      if(u1 <= a)
      {
        if(data_filtered$new_survival[e] <= (76.1*12 - data_filtered$Age_Diagnosis[e]*12))
        {
          data_filtered$new_survival_early[e] <- 76.1*12 - data_filtered$Age_Diagnosis[e]*12
        }
      }
    }
    ...
    # a. Average # of white males 55-59 8366815 (average census population size from 2010-2016)
    ...
    df_pancreas4[t, 1] <- x
    df_pancreas4[t, 2] <- a
    df_pancreas4[t, 3] <- estimate
    df_pancreas4[t, 4] <- estimate*1825 - (8366815-1825)*x*0.03*23.6*12 # 23.6 = 76.1 - average of 55-59
    t <- t + 1
  }
}
```
...
```{r, fig.width = 10}
ggplot(as.data.frame(df_pancreas4), aes(x = V1*100, y = V2, fill = V4/8366815*30))+
  geom_tile()+
  ...
  scale_x_continuous(expand = c(0,0), breaks = seq(0, 0.1, by = 0.025))+
  scale_y_continuous(expand = c(0,0), breaks = seq(0.25, 1, by = 0.25))+
  theme(axis.text.x = element_text(size = 32),
        strip.text = element_text(size = 32),
        axis.title = element_text( size = 32),
        ...
        title = element_text(size = 32),
        legend.key.height = unit(2.5, "cm"))+
  labs(fill="Average Net Survival\nBenefit Per Screen\n(Days)") 
```
...
df_pancreas5 <- matrix(0, length(seq(0, 0.001, by = 0.00001))*length(seq(0, 1, by = 0.01)), 6)
x_i <- 1
for(p in seq(0, 0.001, by = 0.00001)) #probability of treatment induced mortality (mortality = 1 month)
{
  for(a in seq(0, 1, by = 0.01))
  {
    ...
    df_pancreas5[x_i, 2] <- a
    df_pancreas5[x_i, 3] <- estimate
    df_pancreas5[x_i, 4] <- estimate2
    df_pancreas5[x_i, 5] <- m3
    df_pancreas5[x_i, 6] <- m3_mort
    x_i <- x_i + 1
  }
}
```
...
```{r, fig.width = 10}
ggplot(as.data.frame(df_pancreas5), aes(x = V1*100, y = V2, fill = V4/12))+
  geom_tile()+
...
for(i in 1:nrow(plot_data_pancreas75))
{
  for(j in seq(0.8, 1, by = 0.05))
  {
    ...
    df_pancreas6[x_i, 1] <- plot_data_pancreas75$age[i]
    df_pancreas6[x_i, 2] <- plot_data_pancreas75$race[i]
    df_pancreas6[x_i, 3] <- plot_data_pancreas75$sex[i]
    df_pancreas6[x_i, 4] <- j
    df_pancreas6[x_i, 5] <- estimate
    x_i <- x_i + 1
  }
}
...
h_data_full$order <- ifelse(h_data_full$data == "Empirical", "1", "2")
...
N1_data_full$order <- ifelse(N1_data_full$data == "Empirical", "1", "2")
...
N2_data_full$order <- ifelse(N2_data_full$data == "Empirical", "1", "2")
...
fills <- c("1" = "steelblue", "2" = "salmon")
ggplot() +
  geom_density(data = h_data_full_c, aes(x = n, fill = order), alpha = 0.5, bw = 10)+
  facet_wrap(~age+sex, ncol=6)+
  scale_fill_manual(name = "Distribution:", values = fills, breaks=c("1", "2"), labels=c("Empirical", "Assumed"))+ 
...
ggplot() +
  geom_density(data = h_data_full_aa, aes(x = n, fill = order), alpha = 0.5, bw = 10)+
  facet_wrap(~age+sex, ncol=6)+
  scale_fill_manual(name = "Distribution:", values = fills, breaks=c("1", "2"), labels=c("Empirical", "Assumed"))+ 
...
ggplot() +
  geom_density(data = h_data_full_ap, aes(x = n, fill = order), alpha = 0.5, bw = 10)+
  facet_wrap(~age+sex, ncol=6)+
  scale_fill_manual(name = "Distribution:", values = fills, breaks=c("1", "2"), labels=c("Empirical", "Assumed"))+ 
...
ggplot() +
  geom_density(data = h_data_full_h, aes(x = n, fill = order), alpha = 0.5, bw = 10)+
  facet_wrap(~age+sex, ncol=6)+
  scale_fill_manual(name = "Distribution:", values = fills, breaks=c("1", "2"), labels=c("Empirical", "Assumed"))+ 
...
ggplot() +
  geom_density(data = N1_data_full_c, aes(x = n, fill = order), alpha = 0.5, bw = 10)+
  facet_wrap(~age+sex, ncol=6)+
  scale_fill_manual(name = "Distribution:", values = fills, breaks=c("1", "2"), labels=c("Empirical", "Predicted"))+ 
...
ggplot() +
  geom_density(data = N1_data_full_aa, aes(x = n, fill = order), alpha = 0.5, bw = 10)+
  facet_wrap(~age+sex, ncol=6)+
  scale_fill_manual(name = "Distribution:", values = fills, breaks=c("1", "2"), labels=c("Empirical", "Predicted"))+ 
...
ggplot() +
  geom_density(data = N1_data_full_ap, aes(x = n, fill = order), alpha = 0.5, bw = 10)+
  facet_wrap(~age+sex, ncol=6)+
  scale_fill_manual(name = "Distribution:", values = fills, breaks=c("1", "2"), labels=c("Empirical", "Predicted"))+ 
...
ggplot() +
  geom_density(data = N1_data_full_h, aes(x = n, fill = order), alpha = 0.5, bw = 10)+
  facet_wrap(~age+sex, ncol=6)+
  scale_fill_manual(name = "Distribution:", values = fills, breaks=c("1", "2"), labels=c("Empirical", "Predicted"))+ 
...
ggplot() +
  geom_density(data = N2_data_full_c, aes(x = n, fill = order), alpha = 0.5, bw = 10)+
  facet_wrap(~age+sex, ncol=6)+
  scale_fill_manual(name = "Distribution:", values = fills, breaks=c("1", "2"), labels=c("Empirical", "Predicted"))+ 
...
ggplot() +
  geom_density(data = N2_data_full_aa, aes(x = n, fill = order), alpha = 0.5, bw = 10)+
  facet_wrap(~age+sex, ncol=6)+
  scale_fill_manual(name = "Distribution:", values = fills, breaks=c("1", "2"), labels=c("Empirical", "Predicted"))+ 
...
ggplot() +
  geom_density(data = N2_data_full_ap, aes(x = n, fill = order), alpha = 0.5, bw = 10)+
  facet_wrap(~age+sex, ncol=6)+
  scale_fill_manual(name = "Distribution:", values = fills, breaks=c("1", "2"), labels=c("Empirical", "Predicted"))+ 
...
ggplot() +
  geom_density(data = N2_data_full_h, aes(x = n, fill = order), alpha = 0.5, bw = 10)+
  facet_wrap(~age+sex, ncol=6)+
  scale_fill_manual(name = "Distribution:", values = fills, breaks=c("1", "2"), labels=c("Empirical", "Predicted"))+ 
...
INDIVIDUAL ASSUMPTION PLOTS FOR FIGURES

```{r, fig.width=10}
fills <- c("1" = "steelblue", "2" = "salmon")
ggplot() +
  geom_density(data = h_data_full_aa %>% filter(age == "75-79 years" & race == "African American" & sex == "Female"), aes(x = n, fill = order), alpha = 0.5, bw = 10)+
  scale_fill_manual(name = "Distribution:", values = fills, breaks=c("1", "2"), labels = c("Empirical", "Assumed"))+ 
  scale_x_continuous(limits=c(30, 150))+
  ggtitle("Do Exogenous Arrivals Constitute a Homogeneous Poisson Point Process\nAfrican American Women 75-79")+
  xlab("Number of New Diagnoses in a Year")+
  ylab("Smoothed Probability Mass")+
  theme_light()+
  theme(title = element_text(size = 32),
...
ggplot() +
  geom_density(data = N1_data_full_aa %>% filter(age == "75-79 years" & race == "African American" & sex == "Female"), aes(x = n, fill = order), alpha = 0.5, bw = 10)+
  scale_fill_manual(name = "Distribution:", values = fills, breaks=c("1", "2"), labels=c("Empirical", "Predicted"))+ 
  scale_x_continuous(limits=c(0, 100))+
  ggtitle(expression(paste("Distribution of ", N[1]^"*","- African American Women 75-79")))+
  xlab("Number of Pancreatic Cancer Sufferers at a Given Time Diagnosed and Awaiting Surgery")+
  ylab("Smoothed Probability Mass")+
  theme_light()+
  theme(title = element_text(size = 32),
...
ggplot() +
  geom_density(data = N2_data_full_aa %>% filter(age == "75-79 years" & race == "African American" & sex == "Female"), aes(x = n, fill = order), alpha = 0.5, bw = 10)+
  scale_fill_manual(name = "Distribution:", values = fills, breaks=c("1", "2"), labels=c("Empirical", "Predicted"))+ 
  scale_x_continuous(limits=c(0, 75))+
  ggtitle(expression(paste("Distribution of ", N[2]^"*","- African American Women 75-79")))+
  xlab("Number of Pancreatic Cancer Sufferers Receiving Surgery in a Given Year")+
  ylab("Smoothed Probability Mass")+
  theme_light()+
  theme(title = element_text(size = 32),
